            ...
            <div className="body font-serif">
                <div className="filter flex justify-between mt-8">
                    <div className="search mx-10 p-4">
                        <input type="text" className="border border-solid rounded-md w-56 h-10 border-black mr-4" value={searchText} data-testid="searchInput" onChange={(e) => {setSearchText(e.target.value)}}/>
                        <button className="bg-slate-200 text-gray-800 px-4 py-2 rounded-md" onClick={() => {
                            const filteredRestaurant = listOfRestaurants.filter((res) => res.info.name.toLowerCase().includes(searchText.toLowerCase()));
                            setFilteredRestaurants(filteredRestaurant);
                        }}>Search</button>
                    </div>
                    <div className="search mr-12 flex items-center">
                            <button className="px-5 py-2 bg-slate-200 text-gray-800 rounded-md" onClick={() =>{
